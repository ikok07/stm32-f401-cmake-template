 ...
 * @param I2CHandle I2C peripheral handle
...
uint8_t I2C_Init(I2C_Handle_t *pI2CHandle) {
    ...
    // Acknowledge control
    pI2CHandle->pI2Cx->CR1 |= (pI2CHandle->I2C_Config.I2C_ACKControl << I2C_CR1_ACK_Pos);
    ...
    // I2C speed
    uint8_t pclk1FreqMHz = get_pclk1_clock() / 1000000;
    pI2CHandle->pI2Cx->CR2 |= (pclk1FreqMHz << I2C_CR2_FREQ_Pos);

    uint16_t clockPeriodNs = (1 / pclk1FreqMHz) * 1000000000;
    uint16_t ccrValue = 0;
    if (pI2CHandle->I2C_Config.I2C_SCLSpeed == I2C_SCL_SPEED_SM) {
        // 100KHz, 50% Duty cycle => Period / 2 = 5000ns
        ccrValue = 5000 / clockPeriodNs;
    } else {
        uint32_t periodNs = (1 / pI2CHandle->I2C_Config.I2C_SCLSpeed) * 1000000000;
...
uint8_t I2C_MasterSendData(I2C_Handle_t *pI2CHandle, uint8_t *pTXBuffer, uint8_t Len, uint16_t SlaveAddr) {
    ...
    while (Len > 0) {
        ...
        while (!(pI2CHandle->pI2Cx->SR1 & I2C_SR1_TXE_Pos));

        pI2CHandle->pI2Cx->DR = *pTXBuffer++;
        Len--;
    }

    // Wait for transmition completion
    while (!(pI2CHandle->pI2Cx->SR1 & I2C_SR1_TXE_Pos) || !(pI2CHandle->pI2Cx->SR1 & I2C_SR1_BTF_Pos));
